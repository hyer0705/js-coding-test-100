 ...
 *  나가는 것을 지켜볼 수 있는 관리자창을 만드기로 했다. 채팅방에 누군가 들어오면 다음 메시지가 출력된다.
 * "[닉네임]님이 들어왔습니다."
 *
 ...
 *  - 채팅방에서 닉네임을 변경한다.
 *
 * 닉네임을 변경할 때는 기존에 채팅방에 출력되어 있는 메시지의 닉네임도 전부 변경된다.
 *
...
function solution(record) {
  ...
  for (let i = 0; i < record.length; i++) {
    const [cmd, userId, nickname] = record[i].split(" ");

    if (cmd !== COMMAND.LEAVE) userHash.set(userId, new User(userId, nickname));

    answer.push([cmd, userId]);
  }

  const res = answer
    .filter(([cmd, _]) => cmd !== COMMAND.CHANGE)
    .map((res) => {
      const [cmd, uid] = res;
      const userNickname = userHash.get(uid).nickname;
      if (cmd === COMMAND.ENTER) {
        return `${userNickname}님이 들어왔습니다.`;
      }

      if (cmd === COMMAND.LEAVE) {
        return `${userNickname}님이 나갔습니다.`;
      }
      return;
    });

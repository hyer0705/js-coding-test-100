...
function solution(progresses, speeds) {
  ...
  let copied = [...progresses];
  for (let i = 0; i < copied.length; i++) {
    copied[i] = Math.ceil((100 - progresses[i]) / speeds[i]);
  }

  let pointer = 0;
  while (pointer < copied.length) {
    const completeDays = copied[pointer];
    let cnt = 0;
    for (let i = pointer; i < copied.length; i++) {
      if (copied[i] <= completeDays) cnt++;
      else break;
    }
    answer.push(cnt);
    pointer += cnt;
